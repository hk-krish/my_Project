...
import { Previous, VerticalValidationWizardTitle } from "../../../../../../Constants";
import { VerticalValidationWizardSubTitle } from "../../../../../../Data/Forms&Table/Forms/FormLayout";
...
const VerticalValidationWizard = () => {
  ...
  const handleNextButton = () => {
    ...
    if (firstName !== "" && lastName !== "" && contact !== "" && email !== "" && state !== "" && zip !== "" && steps === 1) setSteps(2);
    else if (recipientUserName !== "" && userName !== "" && cardNumber !== "" && expirationDate !== "" && cvvNumber !== "" && documentationName && steps === 2) setSteps(3);
    else if (bankName !== "" && feedBack !== "" && steps === 3) setShowFinish(true);
    else return toast.error("please fill out all the fields before moving on to the next step");
  };
  return (
    <Col md="12">
      <Card>
        <CommonCardHeader title={VerticalValidationWizardTitle} span={VerticalValidationWizardSubTitle} />
        <CardBody>
            ...
            <Row className="g-3">
              ...
              <Col xxl="9" xl="8" xs="12">
                <TabContent activeTab={steps}>
                  <TabPane tabId={1}><YourInfoForm formData={formData} updateFormData={updateFormData} /></TabPane>
                  <TabPane tabId={2}><CartInfoForm formData={formData} updateFormData={updateFormData} /></TabPane>
                  <TabPane tabId={3} className="custom-input"><NetBankingForm formData={formData} updateFormData={updateFormData} /></TabPane>
                </TabContent>
