...
import { Previous, StudentValidationFormTitle } from "../../../../../../Constants";
import CommonCardHeader from "../../../../../CoreComponents/CommonCardHeader";
import BasicInfoForm from "./BasicInfoForm";
import ProfileForm from "./ProfileForm";
import SocialLinksForm from "./SocialLinksForm";
import { StudentValidationFormSubTitle } from "../../../../../../Data/Forms&Table/Forms/FormLayout";
...
const StudentValidationForm = () => {
  ...
  const handleSubmitButton = () => toast.success("Successfully Completed");
  ...
  const handleNextButton = () => {
    ...
    if (name !== "" && email !== "" && password !== "" && confirmPassWord !== "" && step === 1) setStep(2);
    else if (projectDescription !== "" && portfolioURL !== "" && imageUpload !== "" && step === 2) setStep(3);
  ...
  return (
    <Col xl="6">
      <Card className="height-equal">
        <CommonCardHeader title={StudentValidationFormTitle} span={StudentValidationFormSubTitle} />
        <CardBody className="custom-input">
          <Form className="form-wizard" onSubmit={(e) => e.preventDefault()}>
            <div className={`tab ${step === 1 ? "d-block" : "d-none"}`}><BasicInfoForm formData={formData} updateFormData={updateFormData} /></div>
            <div className={`tab ${step === 2 ? "d-block" : "d-none"}`}><ProfileForm formData={formData} updateFormData={updateFormData} selectedImage={selectedImage} /></div>
            <div className={`tab ${step === 3 ? "d-block" : "d-none"}`}><SocialLinksForm formData={formData} updateFormData={updateFormData} /></div>
            {step === 4 && <FinishForm />}
